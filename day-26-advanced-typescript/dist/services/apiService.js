 ...
 */
export function filterCollection(items, predicate) {
 ...
 */
export function sortByProperty(items, key, direction = SortDirection.Ascending) {
    const copy = [...items];
 ...
 */
export function paginateCollection(items, page = 1, pageSize = 6) {
...
const seedUsers = [
    new AdminUser(1, "Dr. Eleanor Vance", "e.vance@edusphere.io", UserStatus.Active, "Engineering", { canManageUsers: true, canDeleteRecords: true, canAccessFinancials: true, accessLevel: 5 }, "+1 (555) 019-2834", "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=120&auto=format&fit=crop&q=80", new Date("2025-01-15")),
    new User(2, "Marcus Sterling", "m.sterling@edusphere.io", UserRole.Manager, UserStatus.Active, "Product", "+1 (555) 014-9921", "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=120&auto=format&fit=crop&q=80", new Date("2025-02-10")),
    new User(3, "Sophia Laurent", "s.laurent@edusphere.io", UserRole.Developer, UserStatus.Active, "Design", "+1 (555) 018-4412", "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=120&auto=format&fit=crop&q=80", new Date("2025-02-28")),
    new User(4, "David Kalu", "d.kalu@edusphere.io", UserRole.Developer, UserStatus.Pending, "Engineering", "+1 (555) 017-7723", "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=120&auto=format&fit=crop&q=80", new Date("2025-03-01")),
    new AdminUser(5, "Kavita Rao", "k.rao@edusphere.io", UserStatus.Active, "Operations", { canManageUsers: true, canDeleteRecords: false, canAccessFinancials: false, accessLevel: 2 }, "+1 (555) 012-3345", "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=120&auto=format&fit=crop&q=80", new Date("2025-03-05")),
    new User(6, "Lucas Chen", "l.chen@edusphere.io", UserRole.Guest, UserStatus.Suspended, "Marketing", "+1 (555) 019-8831", "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=120&auto=format&fit=crop&q=80", new Date("2025-03-10"))
];
...
export async function createUserApi(input) {
    ...
    const newId = nextUserIdCounter++;
    let createdUser;
    if (input.role === UserRole.Admin) {
        createdUser = new AdminUser(newId, input.name.trim(), input.email.trim().toLowerCase(), input.status, input.department, {
            canManageUsers: true,
        ...
        createdUser = new User(newId, input.name.trim(), input.email.trim().toLowerCase(), input.role, input.status, input.department, input.phone?.trim());
    }
