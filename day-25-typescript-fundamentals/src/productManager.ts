...
export function getProductById(id: ProductId): Product | undefined {
    return productsDatabase.find((product: Product) => product.id === id);
}
 ...
 */
export function toggleAvailability(id: ProductId): Product | null {
    const targetProduct = productsDatabase.find((p: Product) => p.id === id);
    if (!targetProduct) {
        return null;
    }
...
export function deleteProduct(id: ProductId): boolean {
    ...
    productsDatabase = productsDatabase.filter((p: Product) => p.id !== id);
...
export function filterProducts(filter: ProductFilter): Product[] {
    return productsDatabase.filter((product: Product) => {
        // Category check
        ...
        if (filter.minPrice !== undefined && filter.minPrice !== null && !isNaN(filter.minPrice)) {
            if (product.price < filter.minPrice) return false;
        }

        // Max price check
        if (filter.maxPrice !== undefined && filter.maxPrice !== null && !isNaN(filter.maxPrice)) {
            if (product.price > filter.maxPrice) return false;
        }
        ...
        if (filter.searchTerm && filter.searchTerm.trim() !== '') {
            ...
            const matchesTag = product.tags.some((t: string) => t.toLowerCase().includes(term));
            const matchesSupplier = product.metadata[0].toLowerCase().includes(term);
            if (!matchesName && !matchesTag && !matchesSupplier) return false;
        }
 ...
 */
export function calculateStats(products: Product[] = productsDatabase): ProductStats {
    const totalProducts = products.length;
    const availableProducts = products.filter((p: Product) => p.available).length;

    const totalValuation = products.reduce((acc: number, p: Product) => {
        const stockUnits = p.metadata[1]; // from tuple
        return acc + (p.price * stockUnits);
    }, 0);

    const averagePrice = totalProducts > 0
        ? products.reduce((sum: number, p: Product) => sum + p.price, 0) / totalProducts
        : 0;
 ...
 */
export function applyDiscount(product: Product, discount: DiscountTuple): DiscountResult {
 ...
 */
export function getCompileTimeErrorDemos(): CompileTimeErrorDemo[] {
    return [
        {
            id: "err-1",
